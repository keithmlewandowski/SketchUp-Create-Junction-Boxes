...
def JunctionBox.main(fileDir, jbName, jbType, topElevation, xIterationOffset, yIterationOffset, 
  ...
  if baseWidth == 0 && baseDepth == 0 && baseHeight == 0 
    ...
    if baseIfCircle == 1
      if firstTierWallThickness != 0
        # trueBaseWidth = UI.inputbox([jbName + "\nHow wide is the bottom tier's base (One Side)"])[0].to_f
        ...
        radius = (baseWidth - (2 * firstTierWallThickness)) / 2
  
        # if trueBaseWidth != 0 
        #   trueBase = entities.add_group
        #   hole_base = trueBase.entities.add_circle(center_point, normal_vector, radius + trueBaseWidth)
        #   face_inner = trueBase.entities.add_face(hole_base)
        #   trueBaseHeight = UI.inputbox([jbName + "\nHow thick is the bottom tier's base"])[0].to_f
        #   face_inner.pushpull(trueBaseHeight - 0.001)
        #   elevationOffset = elevationOffset + trueBaseHeight
        # end

        center_point = Geom::Point3d.new(xIterationOffset + (baseWidth / 2), yIterationOffset + (baseWidth / 2), elevationOffset)
        hole_base = $base.entities.add_circle(center_point, normal_vector, radius)
        face_inner = $base.entities.add_face(hole_base)
        face_inner.pushpull(baseHeight)
      end
      ...
      extrudeRect(xIterationOffset, 0 + yIterationOffset, 
                  ...
                  0 + elevationOffset, baseHeight, $base)
      ...
      if firstTierWallThickness != 0
        extrudeRect(xIterationOffset + firstTierWallThickness, yIterationOffset + firstTierWallThickness,
                    ...
                    elevationOffset + baseHeight, -baseHeight, $base)
      end
    end
  end
  ...
  if boxWidth == 0 && boxDepth == 0 && boxHeight == 0
    ...
    if firstTierWallThickness != 0
      ...
      if riserAlignment[0].to_f == 1
        ...
        if baseIfCircle == 1
          if boxIfCircle == 1
            depthOffset = baseWidth - boxWidth - (firstTierWallThickness - boxThickness)
          else
            depthOffset = baseWidth - boxDepth - (firstTierWallThickness - boxThickness)
          end
        else
          if boxIfCircle == 1
            depthOffset = baseDepth - boxWidth - (firstTierWallThickness - boxThickness)
          else
            depthOffset = baseDepth - boxDepth - (firstTierWallThickness - boxThickness)
          end
        end
      # bottom
      elsif riserAlignment[0].to_f == 2
        widthOffset = (baseWidth-boxWidth) / 2
        depthOffset = (firstTierWallThickness - boxThickness)
      # left
        ...
        widthOffset = (firstTierWallThickness - boxThickness)
      # right
        ...
        widthOffset = baseWidth - boxWidth - (firstTierWallThickness - boxThickness)
      # center
    ...
    if boxIfCircle == 1
      # box inner circle
      center_point = Geom::Point3d.new(xIterationOffset + widthOffset + (boxWidth / 2), yIterationOffset + widthOffset + (boxWidth / 2), elevationOffset + baseHeight + boxHeight)
  ...
  boxWidth_Bottom = baseWidth
  boxDepth_Bottom = baseDepth
  boxHeight_Bottom = baseHeight
  ...
  if lidHoleWidth == 0 && lidHoleDepth == 0 && lidHeight == 0
    ...
    if boxIfCircle == 1
      ...
      zHoleCenter = baseHeight + boxHeight + elevationOffset + lidHeight
